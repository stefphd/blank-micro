...
class MCP492X {
  public:
    ...
    // `MCP492X myDac(pinNumber);`//SPI is used by defualt
    // `MCP492X myDac(pinNumber, latchPin);`//SPI is used by defualt
	// `MCP492X myDac(&SPI1, pinNumber);`//with SPI1, SPI2 etc.
	// `MCP492X myDac(&SPI1, pinNumber, latchPin);`//with SPI1, SPI2 etc.
    MCP492X(uint8_t);
	MCP492X(uint8_t, uint8_t); //added by SL
	MCP492X(SPIClass*,uint32_t, uint8_t); //added by SL
	MCP492X(SPIClass*, uint32_t, uint8_t, uint8_t); //added by SL
  ...
  private:
	...
	uint8_t _latchPin; //added by SL

    // SPI settings for this chip, set up in begin()
    SPISettings _spiSettings;
	uint32_t _spiSpeed;

	// SPI object, added by SL
	SPIClass* _spi; 
